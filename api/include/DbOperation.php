...
class DbOperation
{
    ...
    function __construct()
    {
		...
		require_once dirname(__FILE__) . '/../../helper/Encryption.php';
        $db = new DbConnect();
        $this->con = $db->connect();
    }
	
	 //Method to let a participantLogin log in
    public function participantLogin($username,$pass){
        $password = Encryption::encrypt($pass);
        $stmt = $this->con->prepare("SELECT * FROM elearn_pc_participant WHERE username=? and password=?");
	...
	public function getparticipant($username){
        $stmt = $this->con->prepare("SELECT * FROM elearn_pc_participant WHERE username=?");
	 ...
	 //Method to fetch all particpant from database
    public function getAllparticipant(){
        $stmt = $this->con->prepare("SELECT * FROM elearn_pc_participant");
        $stmt->execute();
	 ...
	 //Checking the Api Keys is valid or not by api key
    public function isValidApiKey($api_key){
        $stmt = $this->con->prepare("SELECT id from elearn_pc_participant WHERE api_key=?");
