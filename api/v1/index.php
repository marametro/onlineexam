 ...
 * Method: POST
 * */
$app->post('/participantlogin', function () use ($app) {
    verifyRequiredParams(array('username', 'password'));
    $username = $app->request->post('username');
    $password = $app->request->post('password');
    $db = new DbOperation();
    $response = array();
    if ($db->participantLogin($username, $password)) {
        $participant = $db->getparticipant($username);
        $response['error'] = false;
        $response['id'] = $participant['id'];
        $response['name'] = $participant['name'];
        $response['username'] = $participant['username'];
        $response['apikey'] = $participant['api_key'];
		if (!isset($_SESSION)) {
            session_start();
        }
		$_SESSION['login'] = TRUE;
		...
		$_SESSION['fullname']=$participant['name'];
		$_SESSION['api_key']=$participant['api_key'];
...
/* *
 * URL: http://localhost:7777/onlineexam/api/v1/participants
 * Parameters: none
